...
describe('PaystackProvider', () => {
            ...
            get: jest.fn((key: string) => {
              switch (key) {
                case 'PAYSTACK_SECRET_KEY':
                  return 'sk_test_mock_secret_key';
                case 'PAYSTACK_PUBLIC_KEY':
                  return 'pk_test_mock_public_key';
                default:
                  return null;
              }
            }),
  ...
  describe('isAvailable', () => {
    it('should return true when API keys are configured', () => {
      ...
      expect(configService.get).toHaveBeenCalledWith('PAYSTACK_SECRET_KEY');
      expect(configService.get).toHaveBeenCalledWith('PAYSTACK_PUBLIC_KEY');
    });

    it('should return false when secret key is missing', () => {
      (configService.get as jest.Mock).mockImplementation((key: string) => {
        if (key === 'PAYSTACK_SECRET_KEY') return null;
        if (key === 'PAYSTACK_PUBLIC_KEY') return 'pk_test_mock_public_key';
        return null;
      });
    ...
    it('should return false when public key is missing', () => {
      (configService.get as jest.Mock).mockImplementation((key: string) => {
        if (key === 'PAYSTACK_SECRET_KEY') return 'sk_test_mock_secret_key';
        if (key === 'PAYSTACK_PUBLIC_KEY') return null;
        return null;
      });
  ...
  describe('getConfigRequirements', () => {
    it('should return required configuration keys', () => {
      ...
      expect(requirements).toContain('PAYSTACK_SECRET_KEY');
      expect(requirements).toContain('PAYSTACK_PUBLIC_KEY');
    });
  });
  ...
  describe('generateBankDetails', () => {
    it('should generate mock bank details', async () => {
      ...
      expect(result).toHaveProperty('bankName', 'Providus Bank');
      expect(result).toHaveProperty('accountNumber');
      expect(result).toHaveProperty('accountName', 'PAYSTACK-BUSINESS_NAME');
    ...
    it('should handle different customer names', async () => {
      ...
      expect(result.accountName).toBe('PAYSTACK-BUSINESS_NAME');
    });
    ...
    it('should handle custom expiry minutes', async () => {
      ...
      const expectedExpiryTime = Date.now() + 120 * 60 * 1000;
