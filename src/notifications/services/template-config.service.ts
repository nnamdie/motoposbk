    ...
    try {
      ...
      for (const file of jsonFiles) {
        try {
          ...
          this.logger.error(
            `Failed to load template from ${file}: ${error.message}`,
          );
        }

        this.cacheInitialized = true;
        this.logger.log(
          `Template cache initialized. Loaded ${this.templateCache.size} templates`,
        );
      }
    } catch (error) {
      this.logger.error(
        `Failed to initialize template cache: ${error.message}`,
      );
